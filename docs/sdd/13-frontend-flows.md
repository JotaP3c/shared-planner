...
| Consultar financeiro | CURRENT | calendário autorizado → preset/custom → resumo de receitas → loading/empty/error/retry; sem despesas/relatórios |
...
Fluxos protegidos usam `authGuard`; autorização fina permanece no backend. A navegação Finance é derivada de identidade atual + calendários/capacidades, e a rota direta oferece no-access sem substituir o 403 da API. O interceptor anexa Bearer somente a `/api` same-origin, preserva Authorization explícito, limpa/redireciona uma única vez em 401 e preserva sessão em 403/500. Busca global (>=2 caracteres, debounce) navega ao evento no calendário. SSR exige proteções ao acessar localStorage, aplicadas nos serviços de autenticação.
