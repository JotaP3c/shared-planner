...
O `docker compose up --build -d --wait` foi repetido a partir dos HEADs standalone já commitados; os dois rebuilds passaram, as imagens usadas pelos containers corresponderam às tags e os três serviços voltaram `healthy`. O build SSR repetiu somente os três warnings conhecidos de budget SCSS. O `npm audit` reporta 28 vulnerabilidades; elas permanecem dívida técnica explícita e não invalidaram o build/teste desta estabilização. O commit de integração é a própria entrega que contém esta spec e seu SHA é reportado no handoff.
